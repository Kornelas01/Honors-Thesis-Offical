...
January
```{r}
ggplot(FFD_weather, aes(FFD, January, , color = Scientific_name)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

jan_lm <- lm(FFD ~ January, data = FFD_weather)
summary(jan_lm)
```

```{r}
FFD_weather %>% group_by(Year_pennie) %>%
  summarise(DOY = mean(DOY_pennie),
            Jan = mean(January),
            March = mean(March)) %>%
  ggplot(., aes(DOY, March)) +
  geom_point() +
  geom_smooth(method = "lm")
```
...
```{r}
ggplot(FFD_weather, aes(FFD, February)) +
  geom_point() +
  geom_smooth(method = "lm")

feb_lm <- lm(FFD ~ February, data = FFD_weather)
summary(feb_lm)
```
March

```{r}
ggplot(FFD_weather, aes(FFD, March)) +
  geom_point() +
  geom_smooth(method = "lm")

march_lm <- lm(FFD ~ March, data = FFD_weather)
summary(march_lm)
```

April
```{r}
ggplot(FFD_weather, aes(April, FFD, color = Scientific_name)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)

april_lm <- lm(FFD ~ April, data = FFD_weather)
summary(april_lm)
```

May
```{r}
ggplot(FFD_weather, aes(May, FFD)) +
  geom_point() +
  geom_smooth(method = "lm")

may_lm <- lm(FFD ~ May, data = FFD_weather)
summary(may_lm)
```
...
Pennie_Start_5 <- Pennie_Timeline_Start |> 
  group_by(Scientific_name) |> 
  count() |> 
  filter(n >= 5)

start_5years <- semi_join(Pennie_Timeline_Start, Pennie_Start_10)

start_5years |> group_by(Scientific_name) |> count()

ggplot(start_5years, aes(x = Year, y = DOY)) +
  geom_point()
...
```{r}
pre_june_species <- start_5years |> 
  filter(DOY <= 152) |> 
  distinct(Scientific_name)

start_5years_prejune <- semi_join(start_5years, pre_june_species) |> 
  filter(DOY < 250)

# number of data points per species
start_5years_prejune |> group_by(Scientific_name) |> count()

# how many species per year
start_5years_prejune |> group_by(Year) |> count()

ggplot(start_5years_prejune, aes(x = Year, y = DOY)) +
  geom_point(aes(color = Scientific_name)) +
  geom_smooth(method = "lm")

# species in reference year
early_species_1970 <- start_5years_prejune |> 
  filter(Year == 1970) |> 
  distinct()

# how many years of data for species in 1970
prejune_5years_1970 <- semi_join(start_5years_prejune, early_species_1970, join_by(Scientific_name)) 

ggplot(prejune_5years_1970, aes(x = Year, y = DOY)) +
  geom_point(aes(color = Scientific_name)) +
  geom_smooth(method = "lm")
```
...
Plot FFD 
```{r}
ggplot(prejune_FFD_mean_year, aes(Year_prejune, mean_FFD)) +
  geom_point() +
  geom_smooth(method = "lm") +
  theme_bw()
...
```{r}
ggplot(FFD_weather, aes(FFD, January)) +
  geom_point() +
  geom_smooth(method = "lm")
...
April
```{r}
ggplot(FFD_weather, aes(FFD, April)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE)
...
May
```{r}
ggplot(FFD_weather, aes(FFD, May)) +
  geom_point() +
  geom_smooth(method = "lm")
...
```{r}
FFD_df_prejun %>%
  summarise(avg = mean(DOY_prejune))
```
